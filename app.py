...
# --------- GRÁFICOS ----------
    ...
    # Adicionar os traços dos reservatórios (mesmo código anterior)
    ...
    fig.update_layout(
        xaxis=dict(
            rangeslider=dict(
                visible=True,
                thickness=0.08,  # Mais fino
                bgcolor='rgba(150,150,150,0.2)',  # Fundo semi-transparente
                bordercolor='rgba(0,0,0,0.2)',
                borderwidth=1
            ),
            # Remove todos os controles extras
            rangeselector=None,
            type='date'
        ),
    ...
    # Configuração adicional para comportamento do slider
    fig.update_layout(
        dragmode='pan',
        xaxis=dict(fixedrange=False)  # Permite zoom
    )
    
    st.plotly_chart(fig, use_container_width=True, config={
        "displaylogo": False,
        "scrollZoom": True,  # Permite zoom com scroll
        "modeBarButtonsToRemove": ['zoom2d', 'zoomIn2d', 'zoomOut2d']  # Remove botões de zoom
    })
